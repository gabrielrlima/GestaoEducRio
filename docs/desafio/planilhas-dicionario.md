...
**Achado — "vagas ociosas" só é calculável para unidades parceiras, não pra rede toda:**
- `Parceiras2025.xlsx` (aba `MAIO -2025`) traz, por unidade × grupamento, as colunas `Meta` (capacidade), `Aluno` (matriculados) e **`Vagas`** já calculada = `Meta − Aluno`. Confirmado numericamente: unidade `1006`, grupamento BII → `Meta=39, Aluno=36, Vagas=3` (39−36=3 ✓); unidade `1010` → `Meta=103, Aluno=102, Vagas=1` ✓. Nos anos 2021–2024 não vem uma coluna `Vagas` pronta, mas a coluna `Meta` existe em todos, então dá pra calcular `Meta − Aluno` manualmente.
- `totalalunoscreche{2021..2025}.xlsx` (unidades **diretas/públicas** — a maioria da rede, 855 das 872 segundo `briefing-oficial-sme.md`) só tem `Aluno` e `Turma` — **não existe nenhum campo de capacidade/meta pra essas unidades em nenhum arquivo do dataset fornecido**. Não dá pra calcular vaga ociosa real pra elas, só estimar indiretamente (`Turma` × capacidade legal por grupamento, que não vem no dado — seria premissa externa a documentar explicitamente se usada).
- **Conclusão prática:** cruzar vagas ociosas de verdade cobre só as unidades parceiras/conveniadas — um subconjunto pequeno da rede (a contagem exata diverge entre fontes: 372 "Creche Parceira" em `Unidades_Unificadas` vs. 17 = 10 conveniadas + 7 parceria em `briefing-oficial-sme.md`, provavelmente por causa de janelas temporais/status diferentes — não reconciliado). Não dá pra afirmar "vagas ociosas da rede inteira" com o dataset fornecido, só das unidades parceiras.
