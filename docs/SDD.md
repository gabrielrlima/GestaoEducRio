...
## 1. ✅ Decisão tomada: Eixo 2 — Inscrição e Classificação

**Decidido em 2026-08-30** (registrado em `CLAUDE.md` § Decisões). Formato de entrega: **ferramenta operacional** — cadastro de unidades de creche, gestão de vagas por unidade/grupamento/turno, e portal para o responsável cadastrar filhos e inscrevê-los em até 5 unidades (fluxo equivalente ao matricula.rio real). Ataca diretamente **R2** (escolha sem critério territorial) e **R8** (classificação por unidade em vez de por CPF). Papel da IA dentro do produto ainda **TBD** — ver `CLAUDE.md` § Decisões.

Specs candidatos dos eixos 1 e 3 abaixo ficam só como **referência histórica** (não escolhidos) — não implementar a menos que sobre tempo real e o eixo 2 já esteja com MVP publicado.

| | ~~Eixo 1 — Planejamento preditivo territorial~~ (não escolhido) | **✅ Eixo 2 — Inscrição e Classificação (ESCOLHIDO)** | ~~Eixo 3 — Convocação automatizada~~ (não escolhido) |
|---|---|---|---|
| **O que é (MVP ~6h)** | ~~Dashboard por CRE/bairro cruzando demanda histórica (Query A), oferta de vagas (OferecimentosEvagas) e nascidos vivos defasados, gerando ranking de "gap projetado" demanda×oferta. Mapa por microárea é stretch goal.~~ | Simulador retroativo sobre dados históricos: (a) recomendador de unidades por proximidade real, ou (b) reprocessamento de Query A por CPF/família escolhendo a melhor entre até 5 opções em vez de ofertar todas. Ataca R2 e R8. **Escopo real decidido é mais amplo**: ferramenta operacional completa (cadastro de unidades + gestão de vagas + portal de inscrição do responsável), não só o simulador — ver `CLAUDE.md` § Decisões. | ~~Painel operacional que modela o fluxo de convocação (R11-R17) como workflow assistido: checklist de prazos (3 tentativas/3 dias + 3 dias úteis de resposta + extensão), alerta de vencimento, sugestão de próxima ação.~~ |
...
| **Papel da IA** | ~~Resumo executivo em linguagem natural por CRE/bairro explicando o gap; copiloto de cenário conversacional; normalização bairro→microárea.~~ | Explicação em linguagem natural de por que uma família foi/seria classificada em cada unidade; auditor de inscrições com alta chance de cancelamento por distância. **TBD — decisão final pendente.** | ~~Geração de mensagens de convocação personalizadas por canal; decisão da próxima ação dado o estado do caso.~~ |
| **Risco técnico** | ~~Alto: geoprocessamento sem lib madura no stack, endereço ofuscado a nível bairro/CEP.~~ | **Médio-alto**: regra de desempate/pontuação completa não está 100% especificada em `regras-negocio.md` (precisa assumir premissa simplificada e documentá-la como tal); quebra de régua de pontuação 2023→2024 (e agora 2024→2025, ver R28) exige tratamento explícito para não gerar número espúrio. | ~~Alto, de outro tipo: pouco apoio em dado real, histórico de tentativas precisaria ser sintético/mockado.~~ |
| **Formato de entrega natural** | ~~Dashboard analítico + mapa.~~ | **Ferramenta operacional** (decidido — não é só simulação). | ~~Ferramenta operacional tipo painel de casos.~~ |
