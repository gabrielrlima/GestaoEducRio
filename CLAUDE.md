...
- **Endereços do responsável**: cadastra até 3 — residencial (obrigatório), trabalho e alternativo (opcionais, campos extra em `responsavel` prefixados `trabalho_`/`alternativo_`, não uma tabela separada). Decidido em 2026-08-30. **Os três são geocodificados** (colunas `*_latitude`/`*_longitude`) e **todos** entram no motor de recomendação, não só o residencial — é o que permite calcular distância por endereço e desvio de rota casa→trabalho. Uma tabela `endereco_responsavel` chegou a existir na branch `claude/agente-recomendacao` e foi descartada em favor deste modelo, que já tinha UI no portal.
